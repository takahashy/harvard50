...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for var, words in self.domains.items():
            self.domains[var] = set([word for word in words if len(word) == var.length])
    ...
    def revise(self, x, y):
        ...
        for xword in x_domains:
            satisfy = False
            i, j = self.crossword.overlaps[x, y]
            for yword in (self.domains[y] - set(xword)):
                if xword[i] == yword[j]:
                    satisfy = True
                    break
        ...
        while q:
            ...
            if self.revise(var1, var2):
                if len(self.domains[var1]) == 0:
                    return False
                for neigh in self.crossword.neighbors(var1):
                    if neigh != var2:
                        q.append((neigh, var1))
    ...
    def order_domain_values(self, var, assignment):
        ...
        ordered_domain.sort(key=rules_out)
